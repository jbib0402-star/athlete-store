...
type LotteryProduct = {
  ...
  name: string;
  special_type: string;
  lottery_prizes: Prize[] | null;
  lottery_daily_limit: number | null;
...
function prizeRowsHtml(prizes: Prize[]) {
  const rows = prizes.length ? prizes : DEFAULT_PRIZES;
...
function ScratchTicket({ result, onClose }: { result: ScratchResult; onClose: () => void }) {
  ...
  const cellsRef = useRef(new Set<string>());
...
export default function LotteryEnhancer() {
  ...
  const [products, setProducts] = useState<LotteryProduct[]>([]);
  ...
  const loadProducts = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("products")
      .select("id,name,special_type,lottery_prizes,lottery_daily_limit,effect_duration_hours")
      .order("created_at", { ascending: false });
    if (data) setProducts(data as LotteryProduct[]);
  }, [supabase]);
  ...
  useEffect(() => {
    const rememberEdit = (event: MouseEvent) => {
      const target = event.target as Element | null;
      const button = target?.closest(".admin-product-edit-trigger");
      if (!button) return;
      editingProductIdRef.current = button.closest<HTMLElement>(".product-admin-row")?.dataset.productAdminId || null;
    };
  ...
  useEffect(() => {
    const setTypeVisibility = (typeSelect: HTMLSelectElement, configBox: HTMLElement, timedBox: HTMLElement | null, clearTimed: boolean) => {
      ...
      if (clearTimed && type !== "timed" && timedBox) {
        const duration = timedBox.querySelector<HTMLSelectElement>("select");
        const text = timedBox.querySelector<HTMLInputElement>("input[type='text']");
    ...
    const scan = () => {
      ...
      document.querySelectorAll<HTMLElement>(".admin-form").forEach(section => {
        ...
        if (!form) return;
        let wrapper = form.querySelector<HTMLElement>(".lottery-create-fields");
        if (!wrapper) {
          wrapper = document.createElement("div");
          wrapper.className = "lottery-admin-fields lottery-create-fields";
          wrapper.innerHTML = `
            <div class="lottery-admin-title">아이템 유형</div>
            <label>유형 선택
              <select class="lottery-item-type">
                <option value="standard">일반 아이템</option>
                <option value="timed">시간 효과 아이템</option>
                <option value="lottery">일일복권 · 하루 3회</option>
              </select>
            </label>
            <div class="lottery-config-box" hidden>
              <div class="lottery-config-title"><strong>복권 당첨 설정</strong><span>확률 합계 100%</span></div>
              <div class="lottery-prize-head"><span>#</span><span>결과 문구</span><span>포인트</span><span>확률</span></div>
              <div class="lottery-prize-rows">${prizeRowsHtml(DEFAULT_PRIZES)}</div>
              <p>복권 결과는 서버에서 무작위로 결정됩니다. 한 계정은 한국시간 기준 하루 최대 3회 사용할 수 있습니다.</p>
            </div>`;
          const timedBox = form.querySelector<HTMLElement>(".timed-effect-admin-fields");
          if (timedBox) form.insertBefore(wrapper, timedBox); else form.prepend(wrapper);
          const typeSelect = wrapper.querySelector<HTMLSelectElement>(".lottery-item-type")!;
          const configBox = wrapper.querySelector<HTMLElement>(".lottery-config-box")!;
          typeSelect.onchange = () => setTypeVisibility(typeSelect, configBox, form.querySelector<HTMLElement>(".timed-effect-admin-fields"), true);
        }
        const typeSelect = wrapper.querySelector<HTMLSelectElement>(".lottery-item-type")!;
        const configBox = wrapper.querySelector<HTMLElement>(".lottery-config-box")!;
        setTypeVisibility(typeSelect, configBox, form.querySelector<HTMLElement>(".timed-effect-admin-fields"), false);
      });

      const modal = document.querySelector<HTMLElement>(".product-edit-modal");
      if (modal && !modal.querySelector(".lottery-edit-fields")) {
        ...
        if (product) {
          ...
          wrapper.innerHTML = `
            ...
            <label>유형 선택
              <select class="lottery-item-type">
                <option value="standard" ${type === "standard" ? "selected" : ""}>일반 아이템</option>
                <option value="timed" ${type === "timed" ? "selected" : ""}>시간 효과 아이템</option>
                <option value="lottery" ${type === "lottery" ? "selected" : ""}>일일복권 · 하루 3회</option>
              </select>
            </label>
            <div class="lottery-config-box" ${type === "lottery" ? "" : "hidden"}>
              <div class="lottery-config-title"><strong>복권 당첨 설정</strong><span>확률 합계 100%</span></div>
              <div class="lottery-prize-head"><span>#</span><span>결과 문구</span><span>포인트</span><span>확률</span></div>
              <div class="lottery-prize-rows">${prizeRowsHtml(product.lottery_prizes || DEFAULT_PRIZES)}</div>
              <p>확률과 당첨 포인트를 수정할 수 있습니다. 복권은 하루 최대 3회 사용됩니다.</p>
            </div>`;
          const timedBox = modal.querySelector<HTMLElement>(".admin-maintenance-effect-box");
          if (timedBox) modal.querySelector("form")?.insertBefore(wrapper, timedBox); else modal.querySelector("form")?.appendChild(wrapper);
          const select = wrapper.querySelector<HTMLSelectElement>(".lottery-item-type")!;
          const configBox = wrapper.querySelector<HTMLElement>(".lottery-config-box")!;
          select.onchange = () => setTypeVisibility(select, configBox, modal.querySelector<HTMLElement>(".admin-maintenance-effect-box"), true);
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes("/rest/v1/rpc/use_inventory_item") && typeof init?.body === "string") {
        try {
          ...
          const inventoryId = String(parsed?.inventory_item_id || "");
          if (inventoryId) {
            ...
            if (inventory?.product_id) {
              ...
              if (product?.special_type === "lottery") {
                ...
                setScratchResult(data as ScratchResult);
                return new Response("null", { status: 200, headers: { "Content-Type": "application/json" } });
              }
            }
          }
        } catch {
          // 일반 아이템 사용 흐름으로 계속 진행합니다.
        }
      }

      const adminUrl = /\/api\/admin\/?(?:\?.*)?$/.test(url);
      if (adminUrl && typeof init?.body === "string") {
        try {
          const body = JSON.parse(init.body);
          if (body?.action === "create_product" || body?.action === "update_product") {
            const isCreate = body.action === "create_product";
            const root = isCreate ? document.querySelector<HTMLElement>(".lottery-create-fields") : document.querySelector<HTMLElement>(".lottery-edit-fields");
            ...
            if (productId) {
              const { data: sessionData } = await supabase.auth.getSession();
              const configResponse = await previousFetch("/api/admin/lottery", {
                method: "POST",
                headers: { "Content-Type": "application/json", Authorization: `Bearer ${sessionData.session?.access_token || ""}` },
                body: JSON.stringify({ action: "set_config", payload: { product_id: productId, special_type: type === "lottery" ? "lottery" : "standard", prizes } })
              });
  ...
  useEffect(() => {
    if (isAdmin && adminPage) loadUsage();
  }, [isAdmin, adminPage, loadUsage]);

  async function resetUsage(group: UsageGroup) {
    ...
    try {
      ...
      const response = await fetch("/api/admin/lottery", {
        ...
        body: JSON.stringify({ action: "reset_today", payload: { user_id: group.user_id, product_id: group.product_id } })
      });
      const data = await response.json();
      ...
      setAdminNotice(error instanceof Error ? error.message : "초기화에 실패했습니다.");
    }
  }
  ...
  return <>
    ...
    {isAdmin && adminPage && createPortal(
      <section className="panel lottery-admin-usage">
        <div className="panel-heading lottery-usage-heading">
          <div><div className="eyebrow">DAILY LOTTERY · ADMIN TEST</div><h2>오늘 복권 이용 현황</h2><p>테스트를 위해 회원별 오늘 이용 횟수만 초기화할 수 있습니다. 이미 받은 당첨 포인트는 유지됩니다.</p></div>
          <button type="button" className="button outline small" disabled={usageLoading} onClick={loadUsage}>{usageLoading ? "불러오는 중…" : "새로고침"}</button>
        </div>
        {adminNotice && <div className="lottery-admin-notice">{adminNotice}</div>}
        {usage.length ? <div className="lottery-usage-list">
          {usage.map(group => <article className="lottery-usage-row" key={group.key}>
            <div><strong>{group.character_name}</strong><span>@{group.username}</span></div>
            <div><strong>{group.product_name}</strong><span>{group.results.join(" · ")}</span></div>
            <b>{group.count} / 3회</b>
            <button type="button" className="button outline small" onClick={() => resetUsage(group)}>오늘 횟수 초기화</button>
          </article>)}
        </div> : <div className="lottery-usage-empty">오늘 사용된 일일복권이 없습니다.</div>}
      </section>,
      adminPage
    )}
  </>;
