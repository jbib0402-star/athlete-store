...
function kstDayBounds() {
  ...
  const startMs = Date.UTC(year, month, day) - 9 * 60 * 60 * 1000;
  return {
...
async function requireAdmin(request: Request) {
  ...
  return { admin, userId: authData.user.id } as const;
}

type Prize = { label: string; points: number; chance: number };

function normalizePrizes(value: unknown): { prizes?: Prize[]; error?: string } {
  if (!Array.isArray(value) || value.length < 1 || value.length > 10) {
    return { error: "복권 당첨 항목은 1개 이상 10개 이하로 설정해주세요." };
  }

  const prizes: Prize[] = [];
  for (const item of value) {
    if (!item || typeof item !== "object") return { error: "복권 당첨 설정을 확인해주세요." };
...
export async function GET(request: Request) {
  try {
    ...
    const { data: rows, error } = await admin
      .from("lottery_plays")
      .select("id,user_id,product_id,result_label,reward_points,played_at")
      .gte("played_at", start)
      .lt("played_at", end)
    ...
    const users = new Map<string, { character_name: string; username: string }>();
    ...
    if (action === "set_config") {
      ...
      if (specialType === "lottery") {
        ...
        const { error } = await admin.from("products").update({
          special_type: "lottery",
          lottery_daily_limit: 3,
          lottery_prizes: normalized.prizes,
