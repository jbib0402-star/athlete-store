alter table public.inventory
  add column if not exists effect_cancelled_at timestamptz;
...
begin
  ...
  if duration_hours = 6 then
    ...
    where user_id = uid
      ...
      and effect_cancelled_at is null
  ...
  update public.inventory
  set used_at = now(), effect_expires_at = next_expires, effect_cancelled_at = null
  where id = inventory_item_id and user_id = uid and used_at is null;
end $$;
